...
<body>
    ...
    <c:forEach var="orderList" items="${orderList}">
        productNames.push('${orderList.proName}'); // 제품 이름 추가
        productQuantities.push(${orderList.totalQuantity}); // 제품 개수 추가
    </c:forEach>
    ...
    var productPieChart = new Chart(ctx, {
        ...
        onClick: function(evt) {
            const activePoints = productPieChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, false);
            if (activePoints.length) {
                const clickedIndex = activePoints[0].index; // 클릭한 데이터의 인덱스
                const clickedProductName = productNames[clickedIndex]; // 클릭한 제품 이름
                const clickedProductId = ''; // 제품 ID를 가져오거나 설정

                // 클릭한 상품에 대한 상세 페이지 URL을 정의
                const detailPageUrl = `/statistics/${statisticsgender}`; // URL 경로를 상품 ID로 구성
                window.location.href = detailPageUrl; // 새 페이지로 이동
            }
        }
    }
});
            ...
            <div id="genderStatsSection" class="hidden">
                ...
                <form action="/statistics/statisticsList" method="POST">
                    ...
                    <br>
						...
						<br>
												<label for="startDates">시작 날짜:</label>
						<input type="date" id="startDates" name="startDate" value="${startDate}">

						<label for="endDates">종료 날짜:</label>
						<input type="date" id="endDates" name="endDate" value="${endDate}">
