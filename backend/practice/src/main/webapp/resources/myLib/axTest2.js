...
// => join 요청: RestController -> 결과 Text return  
...
function rsJoinf() {
	...
	axios.get(url)
		.then(response => {
			console.log(`response joinform successful `);
			document.getElementById("resultArea1").innerHTML = response.data;
		}).catch(err => {
			alert(`response : joinForm fail ${err.message}`);
		});
...
// 2) axiJoin
// multipart 타입 포함
// JS의 내장객체 FormData 사용(JSON이 아닌 일반 JS 객체)
// 요청 headers "Content-Type" 변경
function axiJoin() {
	...
	axios.post(url, formData, { headers: { 'Content-Type': 'Multipart/form-data' } })
		.then(response => {
			alert(`join successful => ${response.data}`);
			rsLoginf();
		}).catch(err => {
			if (err.message == '502') {
				alert("입력오류다 새꺄")
			} else {
				alert(`join fail ${err.message}`);
			}
		});
	document.getElementById("resultArea2").innerHTML = "";
}
